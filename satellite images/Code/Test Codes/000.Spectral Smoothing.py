...
#dt= data_matrix.reshape((rows*cols),bands).T
#
#dt[np.isnan(dt)] = 0    
#oparray= savgol_filter(p1,9,1,mode='interp', axis=0)
#
#new_data_matrix= np.array(oparray.T)
#new_data_matrix= new_data_matrix.reshape(bands,rows,cols)  
#dst_filename = 'smoothed_del_test.tif'
#dataset = gdal.Open(filename, gdal.GA_ReadOnly)
#gdal_datatype = gdal.GDT_Float32
#np_datatype = np.float32
#driver = gdal.GetDriverByName( "GTiff" )
#originX, pixelWidth, b, originY, d, pixelHeight = dataset.GetGeoTransform() 
#dst_ds = driver.Create( dst_filename, new_data_matrix.shape[2], new_data_matrix.shape[1], new_data_matrix.shape[0], gdal_datatype )
#dst_ds.SetGeoTransform((originX, pixelWidth, 0, originY, 0, pixelHeight))
#for i, image in enumerate(new_data_matrix, 1):
#        dst_ds.GetRasterBand(i).WriteArray( image )
#prj=dataset.GetProjection()
#outRasterSRS = osr.SpatialReference(wkt=prj)
#dst_ds.SetProjection(outRasterSRS.ExportToWkt())
